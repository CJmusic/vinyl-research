...
% analysis order:decimate,test_freq,analytic_freq,200Hz_LP,WF_weighting
...
title('decimated file data')
%-----------------plots------------
f=[0:Nt/2]*fs/Nt;
%-------------------get rough estimate of test freq--------------------
REV4TONE=fft(tone);
[~,I]=max(REV4TONE(1:floor(Nt/2+1)));
test_freq=(I-1)*fs/Nt;
...
xlabel('Time[sec]')
ylabel('Freq[Hz]')
title('filtered unweighted freq deviation')
%----------------remove DC from freq---------------------
ns=round(Nt/10);
nf=round(9*Nt/10);
