...
% % ylim([0,400])
% % titlestring = 'Clicks vs. Threshold';
% % title(titlestring)
...
function [csig, clicks] = ClickDetect(sig)
    ...
    sep = 2048; %size of buffer
    s2 = sep/2;
    
    b2 = sig.^2; %power
    ms_seq = b2; 

    for ii = (1:floor(log2(sep))) %loop through the powers of two from highest to lowest based on buffer size
        i = 2^ii; %the index at each power of two
        for j =(1:length(sig)-sep) %loop through the whole signal creating an array that is the power 
            ms_seq(j) = ms_seq(j) + ms_seq(j+i); %ms_seq now diverges from power, and becomes change in power
        end
    end
    ms_seq = ms_seq./sep; %change in power is weighted by the buffer size
    clicks = [];
    left = 0;
    wrc = clickwidth/4; %not sure what this is, some fraction of the click width
    while wrc >= 1 %loop through cutting the wrc in half each time
        wrc = wrc/2;
        ww = clickwidth/wrc; 
        for i = (1:length(sig)-2*sep); %loop through the signal
            msw = 0;
            for j = (1:ww)  %loop through the "click widths"
                msw = msw + b2(i + s2 + j); % sum the power
            end
            msw = msw/ww; %weight by clickwidth/wrc
            if  msw >= threshold*ms_seq(i)/10 % if the msw is greater than the threshold times the change in power no click detected
                clickdetected = 0;
                if left == 0
                    left = i + s2; % move bin over by half a buffer length
                end
            else 
                if(left ~= 0 && floor(i-left+s2) <= ww*2) % if whats left is less than twice the "click widths", click detected and corrected for
                    lv = sig(left); % signal at the left most bound of the click
                    rv = sig(i+ww+s2); % right most bound of the click
                    for j = (left:i+ww+s2)
                        if clickdetected == 0; %log the start of the click while the buffer fixes the audio
                            clicks = [clicks, j];
                            clickdetected = 1;
                        end
                        csig(j) = (rv*(j-left) + lv*(i+ww+s2-j))/(i+ww+s2-left); % do a simple linear interpolation to fix the audio
                        b2(j) = csig(j).^2;
                    end
                    left = 0;
                elseif left ~= 0
