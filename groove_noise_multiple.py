...
def groove_map(file_path, file_name, filter = False): 
    ...
    plt.savefig('plots/' + filename[:-3] + '.png', figsize=(16,9),dpi = 300)
    plt.clf()
...
directory = audio_dir + file_dir
for filename in os.listdir(directory): 
    if filename.endswith(".wav"):
        groove_map(os.path.join(directory,filename), filename, filter = False)
